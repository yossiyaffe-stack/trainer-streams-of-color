...
import { Search, ChevronLeft, ChevronRight, Grid3X3, LayoutGrid, Square, Palette } from 'lucide-react';
...
export function MuseumPaintingsGrid() {
  ...
  const fetchPaintings = async () => {
    ...
    // Fetch only museum imports (those with notes containing "Imported from")
    const { data, error, count } = await supabase
      .from('paintings')
      .select('*', { count: 'exact' })
      .or('notes.ilike.%Imported from%,original_filename.ilike.%AIC-%,original_filename.ilike.%Met-%')
      .order('created_at', { ascending: false });

    if (error) {
      console.error('Error fetching museum paintings:', error);
    } else {
  ...
  }, [paintings, searchQuery, selectedSeason]);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery, selectedSeason, itemsPerPage]);
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 p-4 rounded-xl bg-card border border-border">
        ...
        <span className="text-sm text-muted-foreground">
          {filteredPaintings.length} museum paintings
        </span>
      </div>
      ...
      {paintings.length === 0 && (
        <div className="text-center py-16">
          ...
          <h3 className="font-medium text-lg mb-2">No museum paintings yet</h3>
          <p className="text-muted-foreground">
            Import paintings from museums using the Museum Import tab
          </p>
        </div>
      )}
        ...
        {paginatedPaintings.map((painting, index) => (
          ...
          >
            <div className="relative aspect-[3/4] rounded-lg overflow-hidden border border-border bg-muted hover:border-primary transition-colors">
              ...
              >
                {painting.original_filename?.includes('AIC') ? 'AIC' : 
                 painting.original_filename?.includes('Met') ? 'Met' : '🏛️'}
              </Badge>
            </div>
          </motion.div>
        ))}
